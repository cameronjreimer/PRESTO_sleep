...
#FOR FUTURE REFERENCE ###### If all exercise types are NA in one row, final value is NA. If at least one is NOT NA, all other NAs in that row should be 0. TAKE THE SUM OF ALL non-NAs.
data_full$exercise <- data_full$b_walkexer + data_full$b_bike + data_full$b_jogswimraq + data_full$b_aerobics + data_full$b_freeweights + data_full$b_yoga

#select important columns
data = data_full[c("StudyID", "Country", "ageatqstn", "b_race_ethnic_census", "b_educ", "b_income", "ct_HHinc", "ct_lths", "ct_urban", "pss_score", "b_mdi_score",  "b_finisheddate", "b_hourssleep", "b_currentjob", "b_jobhrsperweek","exercise", generate_all_ndvi_colnames(years,buffers))]
rm(data_full)
